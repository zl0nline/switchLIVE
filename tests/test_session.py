...
class TestPrompts:
    ...
    def test_match_login_failed(self):
        assert match_login_failed("Login invalid") is True
        assert match_login_failed("Access denied") is True
        assert match_login_failed("Authentication failed") is True
        assert match_login_failed("Welcome") is False
...
class TestCLISession:
    ...
    def test_login_with_credentials(self):
        """Полный цикл: login → password → prompt."""
        t = MockTransport()
    ...
    def test_login_enable_password(self):
        ...
        t.add_response(b"Login: ")
        t.add_response(b"Password: ")
        t.add_response(b"Password: ")  # enable
        t.add_response(b"switch:#")
        t.open()

        session = CLISession(t, vendor="dlink")
        creds = Credentials(username="admin", password="secret", enable_password="enable123")
        assert session.login(creds) is True
        assert session.prompt == "switch:#"
